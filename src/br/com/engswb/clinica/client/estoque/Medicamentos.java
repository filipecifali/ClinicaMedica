...
public class Medicamentos implements EntryPoint {
	...
	public void onModuleLoad() {
		...
		Label labelPesq = new Label("Nome:");
		absolutePanel.add(labelPesq, 10, 85);
		labelPesq.setSize("47px", "18px");
		
		TextBox textBoxPesq = new TextBox();
		absolutePanel.add(textBoxPesq, 56, 85);
		textBoxPesq.setSize("84px", "6px");
		...
		absolutePanel.add(listBoxPesq, 106, 56);
		...
		Button btnPesq = new Button("New button");
		btnPesq.setText("Pesquisar");
		absolutePanel.add(btnPesq, 532, 77);
		btnPesq.setSize("89px", "30px");
    	...
    	rpcServiceForn.consultaFornecedores(new AsyncCallback<List<FornecedoresED>>(){
			...
			public void onSuccess(List<FornecedoresED> listaFornecedores) {
				...
				for(int i=0;i<listaFornecedores.size();i++){  
					...
					listBox_1.addItem(listaFornecedores.get(i).getNome(), (listaFornecedores.get(i).getIdFornecedor()) + "");
				}
			}
	    });
		...
		tabLayoutPanel.add(button, "New Widget", false);
